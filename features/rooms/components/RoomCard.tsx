...
const RoomCard = ({ room }: { room: Room }) => {
  ...
  const { members } = useRoomDetailsStore();

  const role = calculateRole(user?.id ?? null, room, members);
  // console.log('role RoomCard', role);
  // console.log('user RoomCard', user);
  return (
    <div className='group relative bg-zinc-900/40 border border-zinc-800 hover:border-cyan-500/50 rounded-2xl p-5 transition-all duration-300 hover:shadow-[0_8px_30px_rgb(0,0,0,0.12)] hover:-translate-y-1'>
      <div className='flex justify-between items-start mb-10'>
        ...
        {permissions.canDeleteRoom(role) && (
